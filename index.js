...
async function discoverService(serviceName) {
  const intances = await client.getAll().prefix(
    `/services/${serviceName}`
  ).strings()

  return Object.entries(intances).map(([key,value])=> JSON.parse(value))
}
...
(async function main() {
  ...
  watchService(serviceName, updataInstances => {
    console.log('服务节点有变动', updataInstances)
  })
})()
